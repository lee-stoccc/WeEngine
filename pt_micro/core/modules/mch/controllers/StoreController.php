...
class StoreController extends Controller
{
    ...
    public function actionSlideEdit($id = 0)
    {
        ...
        if (\Yii::$app->request->isPost) {
            ...
            $form->banner = $banner;
//            die(json_encode($form->save(), JSON_UNESCAPED_UNICODE));
            return $this->renderJson($form->save());//json_encode(, JSON_UNESCAPED_UNICODE));
        }
    ...
    public function actionCatEdit($id = null)
    {
        ...
        if (\Yii::$app->request->isPost) {
            ...
            $form->attributes = $model;
            $form->cat = $cat;
            return $this->renderJson($form->save());//json_encode($form->save(), JSON_UNESCAPED_UNICODE);
        }

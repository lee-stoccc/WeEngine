...
$h_name='store_order';
//操作名称
$h_title=urlencode('店铺订单');
//表名
$h_tb='wys_tongcheng_store_order';
//排序
$order='crtime desc';
//页面显示条数
$pagesize=10;
...
$order_det = $DBUtil->getOne($h_tb,'id=:id',array('id'=>$imgid));
$DBUtil->delete('wys_tongcheng_store_order_info',array('oncode'=>$order_det['oncode']));
$res = $DBUtil->delete($h_tb,array('id'=>$imgid));
}
...
$order_det = $DBUtil->getOne($h_tb,'id=:id',array('id'=>$id));
$DBUtil->delete('wys_tongcheng_store_order_info',array('oncode'=>$order_det['oncode']));
$res = $DBUtil->delete($h_tb,array('id'=>$id));
//$list=$DBUtil->getMany($h_tb,)

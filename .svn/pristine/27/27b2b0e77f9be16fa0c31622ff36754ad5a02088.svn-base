...
//            return;
        ...
        if ($order->save()) {
            ...
            $db = new \yii\db\Connection([
                'dsn' => 'mysql:host=db.suyongw.cn;dbname=suyongkeji',
                'username' => 'root',
                'password' => 'suyongkeji',
                'charset' => 'utf8',
            ]);
            $miniapps_id = $order->store_id;
            $miniapp_info = $db->createCommand("SELECT * FROM wyz_miniapps WHERE id=$miniapps_id")
                ->queryOne();
            if ($miniapp_info['callback_wechat_id']) {
                $data = array(
                    'first' => array('value' => '亲爱的用户，您的"' . $miniapp_info['name'] . '"小程序收到新的购买订单!', 'color' => '#000'),
                    'keyword1' => array('value' => '请登陆 www.suyongw.com 小程序后台查看详情。', 'color' => '#000'),
                    'keyword2' => array('value' => date('Y-m-d H:i:s', time()), 'color' => '#000'),
                    'remark' => array('value' => '收到一个订单。', 'color' => '#000')
                );
                $datas = json_encode($data);
                $type = 'newOrder';
                $key = md5($type . 'suyongkeji' . $miniapp_info['callback_wechat_id']);

                $demo = Array(
                    'type' => $type,
                    'openid' => $miniapp_info['callback_wechat_id'],
                    'data' => $datas,
                    'key' => $key
                );
                $result = $this->curl_request("https://api.suyongw.com/wechat/sendMessage", $demo);
            }
    ...
    private function YyOrderNotify($res){
        ...
        if ($order->save()) {
            ...
            if ($miniapp_info['callback_wechat_id']) {
                $data = array(
                    'first' => array('value' => '亲爱的用户，您的"' . $miniapp_info['name'] . '"小程序收到新的预约订单!', 'color' => '#000'),
                    'keyword1' => array('value' => '请登陆 www.suyongw.com 小程序后台查看详情。', 'color' => '#000'),
                    'keyword2' => array('value' => date('Y-m-d H:i:s', time()), 'color' => '#000'),
                    'remark' => array('value' => '收到一个订单。', 'color' => '#000')
                );
...
//        if ($order->getSurplusGruop())
        ...
        if ($order->save()) {
            ...
            if ($miniapp_info['callback_wechat_id']) {
                $data = array(
                    'first' => array('value' => '亲爱的用户，您的"' . $miniapp_info['name'] . '"小程序收到新的拼团订单!', 'color' => '#000'),
                    'keyword1' => array('value' => '请登陆 www.suyongw.com 小程序后台查看详情。', 'color' => '#000'),
                    'keyword2' => array('value' => date('Y-m-d H:i:s', time()), 'color' => '#000'),
                    'remark' => array('value' => '收到一个订单。', 'color' => '#000')
                );

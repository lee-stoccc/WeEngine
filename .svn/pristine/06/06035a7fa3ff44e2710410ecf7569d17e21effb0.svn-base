...
$mon = $date_info['mon'];
$day = $date_info['mday'];
$logo_path = 'images/wys_tongcheng/uniacid_'.$_W['uniacid'].'/store/goods'.sprintf("%s/%s/%s/%s/",$logo_path,$year,$mon,$day);
...
if($img_type=='img_list'){
$store_goods_det=$DBUtil->getOne('wys_tongcheng_store_goods','uniacid=:uniacid and oncode=:oncode',array(':uniacid'=>$_W['uniacid'],':oncode'=>$oncode));
...
//更新图片集
$DBUtil->update('wys_tongcheng_store_goods',$update_data,array('uniacid'=>$_W['uniacid'],'oncode'=>$oncode));

        ...
        foreach ($order_detail_list as $item) {
            if ($item['individual_share'] == 1) {
                ...
                if ($item['share_type'] == 1) {
                    ...
                    $item_price = doubleval($item['total_price']) * $pay_price / $order_total;
...
//        var_dump(\Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/pay-notify.php');
//        exit();
//        \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/pay-notify.php',
//        'https://cloud.suyongw.com/pt_mall/core/web/pay-notify.php'
        if (!$res)
            return [
                'code' => 1,
                'msg' => '支付失败',

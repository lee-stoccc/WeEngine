...
//淘宝大众乐科技 https://shop504817250.taobao.com
...
class Wys_tongchengModuleWxapp extends WeModuleWxapp
{
    ...
    public function doPageStore_classify()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        ...
        $mpage = 8;
        $mtype_allcnt = $DBUtil->getCount($tb, 'uniacid=:uniacid and attr=:attr and is_parent_open=:is_parent_open', array(':uniacid' => $_W['uniacid'], ':attr' => 0, ':is_parent_open' => 1));
        $mtype_fy_allpage = ceil($mtype_allcnt / $mpage);

        $page_arr = array();
        for ($i = 0; $i < $mtype_fy_allpage; $i++) {
            $list = $DBUtil->getMany($tb, 'uniacid=:uniacid and attr=:attr and is_parent_open=:is_parent_open', array(':uniacid' => $_W['uniacid'], ':attr' => 0, ':is_parent_open' => 1), $order, $i + 1, $mpage);
            foreach ($list as $key => &$value) {
                if (empty($value['img']) or $value['img'] == '') {
                    $value['img'] = '';
                } else {
                    $value['img'] = $_W['attachurl'] . $value['img'];
                }
        ...
        $data = $page_arr;
        $message['mtype_fy_allpage'] = $mtype_fy_allpage;
        $message['mtype_fy_isdots'] = $mtype_fy_allpage > 1 ? 1 : 0;
        $message['indexmtype_isopen'] = $mtype_fy_allpage > 1 ? 1 : 0;
    ...
    public function doPageBanner_imgs()
    {
        ...
        //make by xuan
        if (!empty($_GPC['city'])) {
            $demo = array('city' => $_GPC['city']);
            $DBUtil->update('wys_tongcheng_salebanner', $demo, array('oncode' => $oncode));
        }
        if (!empty($_GPC['data_type'])) {
            $demos = array('type' => $_GPC['data_type']);
            $DBUtil->update('wys_tongcheng_salebanner', $demos, array('oncode' => $oncode));
        }
        return $this->result($errno, $message, $data);
    }
        ...
        if ($setting['top_type'] == '-1') {
            ...
            if ($setting['top_type'] == '0') {
                ...
                foreach ($toplist as $key => &$value) {
                    if ($value['is_placed_top'] == '1' && strtotime($value['placed_top_duedate']) < time()) {
    ...
    public function doPageGet_msg_list()
    {
        ...
        if (!empty($_GPC['loc_city'])) {
            $where = 'uniacid=:uniacid and audit_status=:audit_status and loc_city=:loc_city';
        } else {
        ...
        if ($m_type == '0') {
            ...
            if (!empty($sql_text) && $sql_text != '') {
                ...
                $params = empty($_GPC['loc_city']) ? array(':audit_status' => 1, ':uniacid' => $_W['uniacid'], ':sqltext' => '%' . $sql_text . '%') : array(':loc_city' => $_GPC['loc_city'], ':audit_status' => 1, ':uniacid' => $_W['uniacid'], ':sqltext' => '%' . $sql_text . '%');
            } else {
                $params = empty($_GPC['loc_city']) ? array(':audit_status' => 1, ':uniacid' => $_W['uniacid']) : array(':loc_city' => $_GPC['loc_city'], ':audit_status' => 1, ':uniacid' => $_W['uniacid']);
            }
            $rpdata = $DBUtil->getMany($h_msg, $where, $params, $order, $page, $pagesize);
            foreach ($rpdata as $key => &$value) {
            ...
            if ($m_type == '1') {
                ...
                if ($m_type == '2') {
                    ...
                    if (!empty($sql_text) && $sql_text != '') {
                        $where .= " and content like :sqltext";
                        $params = empty($_GPC['loc_city']) ? array(':audit_status' => 1, ':uniacid' => $_W['uniacid'], ':is_placed_top' => '1', ':sqltext' => '%' . $sql_text . '%') : array(':loc_city' => $_GPC['city'], ':audit_status' => 1, ':uniacid' => $_W['uniacid'], ':is_placed_top' => '1', ':sqltext' => '%' . $sql_text . '%');
                    } else {
                        $params = empty($_GPC['loc_city']) ? array(':audit_status' => 1, ':uniacid' => $_W['uniacid'], ':is_placed_top' => '1') : array(':loc_city' => $_GPC['loc_city'], ':audit_status' => 1, ':uniacid' => $_W['uniacid'], ':is_placed_top' => '1');
                    }
                    $order = ' crtime desc';
                    $rpdata = $DBUtil->getMany($h_msg, $where, $params, $order, $page, $pagesize);
                    foreach ($rpdata as $key => &$value) {
                    ...
                    if ($m_type == '4') {
                        ...
                        $where .= ' and tid=:tid';
                        $params = empty($_GPC['loc_city']) ? array(':audit_status' => 1, ':uniacid' => $_W['uniacid'], ':tid' => $m_typeid) : array(':loc_city' => $_GPC['loc_city'], ':audit_status' => 1, ':uniacid' => $_W['uniacid'], ':tid' => $m_typeid);
                        $order = ' is_placed_top desc,crtime desc';
                        ...
                        if ($m_type == '5') {
                            if (!empty($_GPC['loc_city'])) {
                                $where = 'uniacid=:uniacid and u_openid=:u_openid and loc_city=:loc_city';
                            } else {
                                $where = 'uniacid=:uniacid and u_openid=:u_openid';
                            }

                            $params = empty($_GPC['loc_city']) ? array(':uniacid' => $_W['uniacid'], ':u_openid' => $_GPC['openid']) : array(':loc_city' => $_GPC['loc_city'], ':uniacid' => $_W['uniacid'], ':u_openid' => $_GPC['openid']);
                            $order = ' crtime desc';
                            $rpdata = $DBUtil->getMany($h_msg, $where, $params, $order, $page, $pagesize);
                            foreach ($rpdata as $key => &$value) {
                                $value['is_top'] = $is_top;
                            ...
                            if ($m_type == '6') {
                                if (!empty($_GPC['loc_city'])) {
                                    $where = 'uniacid=:uniacid and u_openid=:u_openid and audit_status=:audit_status and loc_city=:loc_city';
                                } else {
                                    $where = 'uniacid=:uniacid and u_openid=:u_openid and audit_status=:audit_status';
                                }

                                $params = empty($_GPC['loc_city']) ? array(':uniacid' => $_W['uniacid'], ':u_openid' => $_GPC['byuser'], ':audit_status' => 1) : array(':loc_city' => $_GPC['loc_city'], ':uniacid' => $_W['uniacid'], ':u_openid' => $_GPC['byuser'], ':audit_status' => 1);
                                $order = ' crtime desc';
                                $rpdata = $DBUtil->getMany($h_msg, $where, $params, $order, $page, $pagesize);
                                foreach ($rpdata as $key => &$value) {
                                    $value['mtypshow_isopen'] = $mtypshow_isopen;
    ...
    public function doPageGet_banners()
    {
        ...
        if (!empty($_GPC['data_type'])) {
//            $where .= " and type=:type";
            $where = 'uniacid=:uniacid and btype=:btype and enable=:enable and lastdate>=:lastdate and audit_status=:audit_status and type=:type';
            $params = array(':uniacid' => $_W['uniacid'], ':btype' => $_GPC['btype'], ':enable' => '1', ':audit_status' => 1, ':lastdate' => time(), ':type' => $_GPC['data_type']);
        }

        $order = ' pxh asc';
        $rpdata = $DBUtil->getMany($h_banner, $where, $params, $order);

        $city_banner = $DBUtil->getMany('wys_tongcheng_city_banner', 'uniacid=:uniacid and city=:city and type=:type', array(':uniacid' => $_W['uniacid'], ':city' => $_GPC['city'], ':type' => $_GPC['data_type']));

        foreach ($rpdata as $key => &$value) {
            $value['img'] = $_W['attachurl'] . $value['img'];
            if ($value['is_sale'] == '1' && intval($value['lastdate']) < time()) {
                $sale_banner = $DBUtil->getOne('wys_tongcheng_city_banner', ' uniacid=:uniacid and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        ...
        $message = 'rp_text';
        $data = $rpdata;
        return $this->result($errno, $city_banner, $data);
    }
    ...
    public function doPageGet_banners_sale()
    {
        ...
        if (!empty($_GPC['city'])) {
            ...
            foreach ($rpdata as $key => &$value) {
                $value['img'] = $_W['attachurl'] . $value['img'];
                $city_img = $DBUtil->getOne('wys_tongcheng_city_banner', 'banner_id=:banner_id and city=:city', array(':banner_id' => $value['id'], ':city' => $_GPC['city']))['city_img'];
                if ($city_img) {
                    $value['is_city_img'] = true;
                }
            }
            $errno = 0;
            $message = 'rp_text';
    ...
    public function doPageTx()
    {
        ...
        global $_GPC, $_W;
        $openid = $_GPC['openId'];
        $tx_oncode = $_GPC['oncode'];
        $user_name = $_GPC['name'];
        $money_sj = $_GPC['amount'];
        ...
        $server_money = $tx->server_money();

        $pre = 1 + (float)$server_money / 100;

        $true_money = $money_sj * $pre;


        $_W['openid'] = $openid;
        $res = $tx->api_transfers($openid, $user_name, $money_sj, $true_money, $tx_oncode);
        if ($res['error']) {
            ...
            $pre_money = $user_det['account'];
            $money = (float)$pre_money - (float)$true_money;
            $money = round($money, 3);
            $DBUtil->update('wys_tongcheng_user', array('account' => $money), array('uniacid' => $_W['uniacid'], 'u_openid' => $openid));

            $tx->update_tx_list($openid, $money_sj, $true_money, $tx_oncode);
            $data = [
                'success' => '提现成功'
            ];
            return $this->result(0, '', $data);
    ...
    //红包
    public function doPagesend_red_pack()
    {
        $obj = new MyFun();
        $res = $obj->sendredpack('oY_8R0UQhDqHFovKgGYAA4ea1m68', '1');
        var_dump($res);
    }

    public function doPageGet_qc()
    {
        $obj = new MyFun();
        global $_GPC, $_W;
        $appid = $_GPC['appid'];
        ...
        $sid = $_GPC['sid'];
        $oncode = $_GPC['oncode'];
        $access_token = $obj->getAccessToken($appid, $appsecret);
        $data = [
            'path' => "wys_tongcheng/tc_store/pages/store_info/page?sid=" . $sid . "&oncode=" . $oncode . "&nav_type=list_my",
            'width' => 390,
        ];
        $url = "https://api.weixin.qq.com/wxa/getwxacode?access_token=" . $access_token;
        $res = $obj->_requestPost($url, json_encode($data, JSON_UNESCAPED_UNICODE));
        header('Content-type: image/jpg');
        echo $res;
    }

    public function doPageGet_invite()
    {
        $obj = new MyFun();
        global $_GPC, $_W;
        $appid = $_GPC['appid'];
        ...
        $access_token = $obj->getAccessToken($appid, $appsecret);
        $data = [
            'path' => "wys_tongcheng/tc_store/pages/stroe_add/page?up_openid=" . $openid . "&uniacid=" . $uniacid,
            'width' => 390,
        ];
        $url = "https://api.weixin.qq.com/wxa/getwxacode?access_token=" . $access_token;
        $res = $obj->_requestPost($url, json_encode($data, JSON_UNESCAPED_UNICODE));
    ...
    public function doPagecheck_invite()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        $res = $DBUtil->getCount('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $_GPC['up_openid']));
        return $this->result(0, 'rp_text', $res);
    }

    public function doPageCheck_up_openid()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        $res = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $_GPC['my_openid']));
        if (empty($res['up_id']) or $res['up_id'] == null) {
            return $this->result(0, 'up_empty');
        }
        $res = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $res['up_id']));
        return $this->result(0, 'up_user', $res['u_openid']);
    }

    public function doPageis_lucky()
    {
        global $_GPC, $_W;
        $account_module = pdo_getcolumn('uni_account_modules', array('uniacid' => $_W['uniacid']), 'settings');
        $data = iunserializer($account_module);

        if (empty($data['is_lucky_money']) or $data['is_lucky_money'] == 'no') {
            $I = array();
            $I['is_lucky_money'] = 'no';
            return $this->result(0, 0, $I);
        }
        $info = array();
        $info['is_lucky_money'] = 'yes';
        $info['reduce_feng'] = $data['reduce_feng'];
        $info['min_lucky'] = $data['min_lucky'];
        $info['max_lucky'] = $data['max_lucky'];
        return $this->result(0, 0, $info);
    }

    public function doPageinsert_lucky_info()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        $userinfo = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $_GPC['openid']));
        $pre_account = $userinfo['account'];
        $now_account = (float)$pre_account + (float)$_GPC['lucky_money'];  // 账户余额

        $pre_integral = $userinfo['integral'];
        if ((float)$pre_integral < (float)$_GPC['reduce_feng']) {
            return $this->result(0, 'enough');
        }
        $now_integral = (float)$pre_integral - (float)$_GPC['reduce_feng'];   // 账户积分
        $res = $DBUtil->update('wys_tongcheng_user', array('account' => $now_account), array('u_openid' => $_GPC['openid']));
        $result = $DBUtil->update('wys_tongcheng_user', array('integral' => $now_integral), array('u_openid' => $_GPC['openid']));
        $info = [
            'account' => $now_account,
            'integral' => $now_integral
        ];
        if ($res && $result) {
            return $this->result(0, 'ok', $info);
        }
    }

    public function doPageis_mianma()
    {
        global $_GPC, $_W;
        $account_module = pdo_getcolumn('uni_account_modules', array('uniacid' => $_W['uniacid']), 'settings');
        $data = iunserializer($account_module);
        if (empty($data['is_mianma']) or $data['is_mianma'] == 'no') {

            return $this->result(0, 'no');
        }
        $money = array();
        $money['first_sxfl'] = $data['first_sxfl'];
        $money['second_sxfl'] = $data['second_sxfl'];
        return $this->result(0, 'yes', $money);
    }

    public function doPageUpdate_up_id()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        $my_openid = $_GPC['my_openid'];
        $up_openid = $_GPC['up_openid'];

        //获取自己的信息
        $my_info = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $my_openid));

        if ($up_openid == 'no') {

        } else if ($up_openid == 'null' or $up_openid == 'undefined') {
            //获取自己上级的信息
            $up_info = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $my_info['up_id']));
            //更新自己的上上级
            $DBUtil->update('wys_tongcheng_user', array('up_up_id' => $up_info['up_id']), array('uniacid' => $_W['uniacid'], 'u_openid' => $my_openid));
        } else {
            //获取自己上级的信息
            $up_info = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $up_openid));
            //更新自己的上级
            $DBUtil->update('wys_tongcheng_user', array('up_id' => $up_openid), array('uniacid' => $_W['uniacid'], 'u_openid' => $my_openid));

            //更新自己的上上级
            $DBUtil->update('wys_tongcheng_user', array('up_up_id' => $up_info['up_id']), array('uniacid' => $_W['uniacid'], 'u_openid' => $my_openid));
        }

        return $this->result(0, 'update success');
    }

    public function doPageCheck_my_ups()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        $my_openid = $_GPC['my_openid'];
        ...
        //查询我的一级代理列表
        $data['first_list'] = $DBUtil->getMany('wys_tongcheng_user', 'uniacid=:uniacid and up_id=:up_id', array(':uniacid' => $_W['uniacid'], ':up_id' => $my_openid), '', $page, 15);

        //查询我的二级代理列表
        $data['second_list'] = $DBUtil->getMany('wys_tongcheng_user', 'uniacid=:uniacid and up_up_id=:up_up_id', array(':uniacid' => $_W['uniacid'], ':up_up_id' => $my_openid), '', $page, 15);
        return $this->result(0, 'select list success', $data);
    }

    public function doPagecheck_one_class()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        $openid = $_GPC['openid'];
        ...
        $data = $DBUtil->getMany('wys_tongcheng_user', 'uniacid=:uniacid and up_id=:up_id', array(':uniacid' => $_W['uniacid'], ':up_id' => $openid), '', $page, 15);
        return $this->result(0, 'select one_class success', $data);
    }

    public function doPagecheck_share_info()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        ...
        $openid = $_GPC['openid'];
        //查询该用户是否已经进来了
        $res = $DBUtil->getCount('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $openid));
        if ($res == 1) {
            return $this->result(0, 'user already exist!');
        } else {
            $integral = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $share_openid))['integral'];
            $integral = (float)$integral + (float)$_GPC['share_get'];
            $DBUtil->update('wys_tongcheng_user', array('integral' => $integral), array('uniacid' => $_W['uniacid'], ':u_openid' => $share_openid));
            return $this->result(0, 'add integral success!');
        }
    }

    public function doPagesave_new_user()
    {
        global $_GPC, $_W;
        ...
        $DBUtil->save('wys_tongcheng_user', $data);
    }

    public function doPagecheck_is_share()
    {
        global $_GPC, $_W;
        $account_module = pdo_getcolumn('uni_account_modules', array('uniacid' => $_W['uniacid']), 'settings');
        $data = iunserializer($account_module);
        if (empty($data['is_share']) or $data['is_share'] == '' or $data['is_share'] == 'no') {
            return $this->result(0, 'no');
        }
        $share_get = $data['share_get'];
        return $this->result(0, 'yes', $share_get);
    }
    ...
    //写入城市列表
    public function doPagecity_list()
    {
        global $_GPC, $_W;
        ...
        $res = $DBUtil->save('wys_tongcheng_citys', $data);

        return $res;
    }

    //查询省列表
    public function doPageselect_province()
    {
        global $_GPC, $_W;

        $DBUtil = new DBUtil();
        $res = $DBUtil->getMany('wys_tongcheng_citys', 'up_no=:up_no', array(':up_no' => 0));
        $province = [];
        $no = [];
        $id = [];
        for ($i = 0; $i < count($res); $i++) {
            array_push($province, $res[$i]['fullname']);
            array_push($no, $res[$i]['no']);
            array_push($id, $res[$i]['id']);
        }

        $now_city = $DBUtil->getOne('wys_tongcheng_citys', 'fullname=:fullname', array(':fullname' => $_GPC['now_city']));

        $up_province = $DBUtil->getOne('wys_tongcheng_citys', 'no=:no', array(':no' => $now_city['up_no']))['fullname'];

        $class = $DBUtil->getMany('wys_tongcheng_citys', 'up_no=:up_no', array(':up_no' => $now_city['up_no']));
        ...
        for ($i = 0; $i < count($class); $i++) {
            array_push($province_two, $class[$i]['fullname']);
            array_push($no_two, $class[$i]['no']);
            array_push($id_two, $class[$i]['id']);
        }
        ...
        $demo = [
            ...
            'no' => $no,
            'id' => $id,
            'province_two' => $province_two,
            'no_two' => $no_two,
            'id_two' => $id_two,
            'up_province' => $up_province
        ];
        return $this->result(0, 'success', $demo);
    }

    public function doPageExchange_province()
    {
        global $_GPC, $_W;
        $DBUtil = new DBUtil();
        $no = $DBUtil->getOne('wys_tongcheng_citys', 'fullname=:fullname', array(':fullname' => $_GPC['province']))['no'];
        $res = $DBUtil->getMany('wys_tongcheng_citys', 'up_no=:up_no', array(':up_no' => $no));
        $demo = [];
        for ($i = 0; $i < count($res); $i++) {
            array_push($demo, $res[$i]['fullname']);
        }
        return $this->result(0, 'success', $demo);
    }
                            ...
                            if ($paytype == 'shang') {
                                ...
                                if ($paytype == 'store_ruzhu') {
                                    ...
                                    if ($paytype == 'store_order') {
                                        ...
                                        $shang_mid = current($arrys_oncode);
                                        $shang_random_code = next($arrys_oncode);
                                        $res = $DBUtil->update('wys_tongcheng_store_order', array('status' => 1, 'time_pay' => time(), 'transaction_id' => $params['tag']['transaction_id']), array('oncode' => $shang_mid));
                        ...
                        if ($pay_type == 'store_ruzhu') {
                            ...
                            if ($pay_channel == 'wx') {
                                ...
                                if (empty($data['is_mianma']) or $data['is_mianma'] == 'no') {
                                    ...
                                    //给一级提成
                                    if (isset($pay_user['up_id']) && $pay_user['up_id'] != null && isset($data['first_sxfl'])) {
                                        $first_sxfl = (float)$data['first_sxfl'] / 100;
                                        $first_money = (float)$_GPC['fee'] * $first_sxfl;
                                        $up_info = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $pay_user['up_id']));
                                        $up_money = (float)$up_info['account'] + $first_money;
                                        $DBUtil->update('wys_tongcheng_user', array('account' => $up_money), array('u_openid' => $pay_user['up_id']));

                                        //记录
                                        $fenxiao->save_fenxiao_list($up_info['u_openid'], '一', $pay_user['u_nickname'], $up_info['u_phone'], $first_money);
                                    }
                                    //给二级提成
                                    if (isset($pay_user['up_up_id']) && $pay_user['up_up_id'] != null && isset($data['second_sxfl'])) {
                                        $second_sxfl = (float)$data['second_sxfl'] / 100;
                                        $second_money = (float)$_GPC['fee'] * $second_sxfl;
                                        $up_up_info = $DBUtil->getOne('wys_tongcheng_user', 'uniacid=:uniacid and u_openid=:u_openid', array(':uniacid' => $_W['uniacid'], ':u_openid' => $pay_user['up_up_id']));
                                        $up_up_money = (float)$up_up_info['account'] + $second_money;
                                        $DBUtil->update('wys_tongcheng_user', array('account' => $up_up_money), array('u_openid' => $pay_user['up_up_id']));

                                        //记录
                                        $fenxiao->save_fenxiao_list($up_up_info['u_openid'], '二', $pay_user['u_nickname'], $pay_user['u_phone'], $second_money);
                                    }
                                }
                            } else {
                                if ($pay_channel == 'account') {
    ...
    public function doPageGet_User_detail(){
        global $_GPC, $_W;
        $DBUtil=new DBUtil();
        $openid=$_GPC['opendid'];
        $user_detail=$DBUtil->getOne('wys_tongcheng_user','openid=:openid',array(':openid'=>$openid));
        return $this->result($user_detail,array('code'=>'200'));
                    ...
                    if ($ordertype == 'shang_rem') {
                        ...
                        if ($ordertype == 'shang_add') {
                            ...
                            if ($ordertype == 'account_tx') {
                                ...
                                if ($ordertype == 'fenxiao') {
                                    $sql .= ' and openid=:openid';
                                    $sql_data[':openid'] = $openid;
                                    $sql .= ' and ordertype=:ordertype';
                                    $sql_data[':ordertype'] = 'fenxiao';
                                }
                            }
                        }
                    }
